...
import 'package:inum/core/constants/enums/button_size_enum.dart';
import 'package:inum/core/constants/enums/button_type_enum.dart';
...
class CustomButton extends StatelessWidget {
  const CustomButton({
    ...
    this.buttonSize = ButtonSizeEnum.medium, this.buttonType = ButtonTypeEnum.primary,
    this.isFullWidth = false, this.icon, this.isLoading = false, this.borderRadius,
  });
  ...
  Widget build(BuildContext context) {
    ...
    ButtonStyle buttonStyle = switch (buttonType) {
      ButtonTypeEnum.primary => ElevatedButton.styleFrom(
          backgroundColor: theme.primaryColor, foregroundColor: white, elevation: 0,
      ...
      ButtonTypeEnum.secondary => ElevatedButton.styleFrom(
          backgroundColor: white, foregroundColor: theme.primaryColor, elevation: 0,
          padding: const EdgeInsets.symmetric(horizontal: 20),
      ...
      ButtonTypeEnum.outline => TextButton.styleFrom(
    ...
    Color textColor = switch (buttonType) {
      ...
      ButtonTypeEnum.outline => theme.primaryColor,
    };

    Widget buttonChild = isLoading
        ? SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2, color: textColor))
        : (icon != null)
            ? Row(mainAxisSize: MainAxisSize.min, children: [icon!, const SizedBox(width: 8), CustomText(text: text, color: textColor)])
